...
식각 공정에서 식각 속도만 높이면 선택비와 균일도가 무너진다. 그래서 이 프로젝트는 세 응답을 동시에 만족하는 조건 범위, 곧 공정 윈도우를 실험 범위 안에서 판정하는 도구를 목표로 삼았다. 실제 장비 데이터는 공개할 수 없어 공개된 silicon RIE 논문의 32-run 실험계획(DOE)을 직접 전사했다. 그 데이터로 21항 반응표면모형(RSM)을 재구성해 논문 계수와 대조했고, 그 위에서 사용자가 입력한 성능 기준을 동시에 만족하는 영역을 탐색하도록 만들었다.
...
| 검증 | 단위 테스트와 UI 테스트 함수 55개, parametrize 전개 후 `68 passed` |
...
제외 해석은 애초에 항별 대조가 성립하지 않는다. 별표가 붙은 8개 run(3, 9, 11, 15, 17, 20, 21, 24)을 빼면 설계행렬이 24×21이지만 rank가 19로 떨어진다. 이때 `np.linalg.lstsq`는 여전히 답을 낸다. 그러나 그 답은 똑같이 잘 맞는 무한히 많은 해 중 최소노름 해 하나일 뿐이다. 그래서 21개 항 중 15개는 개별적으로 식별되지 않고, 개별 대조가 가능한 항은 `b0`, `b1`, `b5`, `b11`, `b55`, `b15` 6개뿐이다. [`scripts/audit_paper_coefficients.py`](scripts/audit_paper_coefficients.py)가 이 rank 부족을 경고로 출력하고 식별 불가능한 항에 `[not identifiable]`을 표시한다. 이 동작은 [`tests/test_audit_paper_coefficients.py`](tests/test_audit_paper_coefficients.py)로 고정했다.
...
곡선은 coded −2~+2 범위 안에서만 유효하다. 범위 검사는 실제값을 coded로 바꾸는 `actual_to_coded`에 있으므로, 이를 거치는 UI 입력과 공정 윈도우 격자에서는 축을 연장한 외삽이 차단된다. 다른 인자를 중심점이 아닌 값에 고정하면 곡선 모양이 바뀌므로 이 그림 하나로 인자별 최적값을 결정할 수 없다. 논문이 제시한 플라즈마 화학 설명과 방향은 일치하지만, 이 분석만으로 새로운 인과관계를 증명한 것은 아니다.
...
2번은 1번과 4번 사이에서 건너뛸 수 없다. 앱의 "Process window" 탭은 원자료로 적합한 모델을 캐시해 두고 slider가 바뀔 때마다 격자만 다시 계산한다. 이 캐시는 2번을 다시 돌려 요약 digest가 바뀔 때만 갱신된다. 반면 "논문 재현 대조" 탭과 "ANOVA" 탭은 2번이 만든 CSV를 읽는다. 두 경로가 서로 다른 버전을 보여 주면 안 되므로 `run_phase_b.py`는 자기 입력(원자료 CSV 2개, 수치 코어 모듈 5개, 스크립트 자신)의 SHA-256을 `phase_b_summary.json`에 기록한다. 앱은 시작할 때 이 값을 다시 계산해 대조하고, 하나라도 어긋나면 어떤 탭도 그리지 않고 멈춘다. 구현은 [`src/etch_window/provenance.py`](src/etch_window/provenance.py)에 있다.
...
├── tests/                          # 단위 테스트와 UI 테스트 함수 55개 (68 passed)
...
| 2026-09-08 (같은 날 코드 리뷰 High 항목) | 함수 55개 / `68 passed` | 요인 민감도, adjusted R², 항별 F 검정, 계수 불일치 18개, 설계행렬 열 순서, 비균일도와 선택비의 예측구간 방향을 고정하는 테스트 추가 |

표의 마지막 두 줄에서 두 수가 갈리는 이유는 `parametrize`로 여러 번 실행되는 테스트가 있기 때문이다. `tests/test_provenance.py`의 한 테스트가 4회, 2026-09-08에 추가한 항별 F 검정과 adjusted R² 테스트가 응답별로 5회씩, 적합결여 검정과 허용오차 경계 테스트가 2회씩 실행된다. `def test_` 함수 수는 55개, pytest 실행 수는 68개이며 둘 다 맞는 값이다.
...
- DOE 범위 밖 외삽은 `actual_to_coded`를 거치는 UI 입력과 격자 경로에서 차단되며, 모델의 `predict`를 coded 값으로 직접 호출하는 경우까지 막지는 않는다. 볼록껍질과 레버리지 한계 밖 조합은 계산은 되지만 확정 판정에서 제외한다.
