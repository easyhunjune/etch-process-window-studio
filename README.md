...
이 값은 각 요인에 속한 선형항, 제곱항, interaction 항을 full model에서 함께 제거했을 때 늘어나는 부분 제곱합을 정규화한 것이다. interaction 항이 두 요인 그룹에 동시에 포함되므로 고전적인 가산형 ANOVA 기여율과 같지 않고, 합도 100%가 되지 않는다. 특히 비균일도의 CHF₃는 기여율 순위만 1위일 뿐 p-value가 0.111이고 모델 전체도 유의하지 않으므로, 지배 인자로 판정하지 않았다.
...
제목에 적은 대로 interaction 항이 두 그룹에 중복 계산되므로 막대의 합은 100%가 아니다. 통계적 유의성과 순위는 별개여서 비균일도의 CHF₃는 p = 0.111로 강한 증거가 아니다. 그래서 이 그림은 어떤 인자를 먼저 확인할지 정하는 우선순위 참고 자료이지 인과관계의 증거가 아니다.
